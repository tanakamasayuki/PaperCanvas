...
| `gencheck/` | **ブラウザツールが出した C++ が本当にコンパイルして動くか。** ツール自身はこのコードを一度も実行しない（プレビューは JS が描く）ので、もっともらしいが誤ったヘッダを出しても、誰かがスケッチに貼るまで気づけない。代表的な生成ヘッダ（レシートとラベル）をここに置いてビルドする。レシート側は**明細を 1 件減らすとページがちょうど 1 行ぶん縮む**ことまで見る（表がツールの見せたとおりに繰り返している証拠）。`MyReceipt.h` / `MyLabel.h` は生成物なので、手で編集せずツールから作り直す |
