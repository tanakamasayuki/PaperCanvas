...
**「全部 u8g2 に変換」も成立しない。** u8g2 の送り幅は 7bit で頭打ちになり、75px の DejaVu72 と Font8 が入らない。

**採るのは「全形式のデコーダを持つ部品ライブラリを別に作る」案。** 決め手は fontgen がブラウザ内で u8g2 を生成することで、**u8g2 の JS デコーダはどのみち必要**になる。そこが決まれば残り 5 形式の追加コストは小さく（u8g2 が一番厄介）、内蔵フォント全部が扱える。

字形の正しさは、**LovyanGFX 自身をオラクルにした一致テスト**で担保する（同じ文字を C++ と JS で描いてバイト比較）。形式仕様の読み間違いという失敗モードは、そこで全部落ちる。
...
- *ホストで 1bpp グリフアトラスを焼いてブラウザが貼る* — 方向は正しかった。§3.4 はこれを一般化したもので、固定アトラスではなくフォント単位のエクスポート経路にした
- *全フォントを u8g2 へ変換して JS は u8g2 デコーダだけ持つ* — CJK は全部 u8g2 なので一見成立するが、75px の GFXfont / RLEfont が u8g2 の 7bit 送り幅に入らない
- *ホストで LovyanGFX に描かせて採取し、JS は中間形式だけ読む* — 全形式を再実装せずに済むが、**ブラウザで生成した u8g2 をプレビューするのに C++ ツールチェーンの往復が要る**。fontgen がブラウザ内で完結する以上、そこを切ってはいけない。この採取の仕組みは §3.4 のとおりテストのオラクルとして残る
...
- フォント部品ライブラリの着手と完成（[FONT_LIBRARY.ja.md](FONT_LIBRARY.ja.md) §7）。**v1.0 までの経路で最も長い依存**。PaperCanvas が必要とするのはデコーダと描画だけなので、そこから作れば全体の完成を待たずに始められる（同 §8）
