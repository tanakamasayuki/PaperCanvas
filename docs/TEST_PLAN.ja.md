...
| `row/` | `addRow`。**API 確定後に詳細を書く**（[CORE_DESIGN.ja.md](CORE_DESIGN.ja.md) §4.2.1 が未確定）。確定によらず必要な観点: **列が行をまたいで縦に揃うこと**（各行の文字長を変えても列位置が動かない）、`%` → px の丸めが決定的であること、リーダー文字の埋め、空セルの扱い、幅不足時に右端の列が切られないこと |
