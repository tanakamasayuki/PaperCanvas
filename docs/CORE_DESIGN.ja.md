...
| マクロ | 既定 | 意味 |
| --- | --- | --- |
| `PAPERCANVAS_NO_DITHER` | 未定義 | Bayer テーブル（80 バイト）を落とす。`Mono::Bayer*` は閾値として振る舞う |

**要素リストは動的確保する。** 検討していた `PAPERCANVAS_MAX_ELEMENTS`（固定長にして動的確保を無くす）は実装していない。要素・テキスト・セル・画素の 4 つのアリーナすべてに同じ仕組みが要るうえ、上限を超えたときの扱いが `add()` の戻り値の意味を変えてしまう。**要求が出てから足す。** 現状の確保は `Arena` に集約してあるので、後から差し替えられる。
